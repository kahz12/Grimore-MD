...
from grimoire.utils.config import _load_project_env


@pytest.fixture
def isolated_env(monkeypatch):
    """Strip GRIMOIRE_TEST_* keys before and after each test."""
    snapshot = {k: v for k, v in os.environ.items() if k.startswith("GRIMOIRE_TEST_")}
    for k in list(os.environ):
        if k.startswith("GRIMOIRE_TEST_"):
...
def test_loads_env_from_cwd(tmp_path, monkeypatch, isolated_env):
    ...
    project.mkdir()
    (project / ".env").write_text("GRIMOIRE_TEST_OWN=one\n")

    monkeypatch.chdir(project)
    assert _load_project_env() is True
    assert os.environ.get("GRIMOIRE_TEST_OWN") == "one"
...
def test_does_not_walk_upward_to_parent_env(tmp_path, monkeypatch, isolated_env):
    ...
    parent.mkdir()
    (parent / ".env").write_text("GRIMOIRE_TEST_PARENT=parent_value\n")
    ...
    assert "GRIMOIRE_TEST_PARENT" not in os.environ


def test_existing_env_var_is_not_overridden(tmp_path, monkeypatch, isolated_env):
    project = tmp_path / "p"
    project.mkdir()
    (project / ".env").write_text("GRIMOIRE_TEST_KEEP=from_dotenv\n")

    monkeypatch.setenv("GRIMOIRE_TEST_KEEP", "from_shell")
    monkeypatch.chdir(project)
    _load_project_env()
    # override=False: shell wins.
    assert os.environ["GRIMOIRE_TEST_KEEP"] == "from_shell"
