...
Grimore SQLite database to prevent data loss.
"""
import os
import shutil
import time
...
class BackupManager:
    ...
    def create_backup(self):
        ...
        backup_path = self.backup_dir / f"grimore_{timestamp}.db"
    ...
    def latest_backup_mtime(self) -> float | None:
        ...
        """
        backups = list(self.backup_dir.glob("grimore_*.db"))
        if not backups:
            return None
        try:
    ...
    def _rotate_backups(self):
        ...
        """
        backups = sorted(self.backup_dir.glob("grimore_*.db"), key=lambda x: x.stat().st_mtime)
