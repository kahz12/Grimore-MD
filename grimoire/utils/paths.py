"""
Cross-platform filesystem paths for Grimoire's runtime artefacts
(daemon lock/log, shell history, …).

Backed by ``platformdirs`` so each OS gets the conventional location
without if/else branches:

  Linux         ~/.cache/grimoire/
  Windows       C:\\Users\\<u>\\AppData\\Local\\grimoire\\Cache\\
  Termux        ~/.cache/grimoire/                (Linux-style)
  macOS         ~/Library/Caches/grimoire/        (incidental)
...
def cache_dir() -> Path:
    """Return Grimoire's user cache directory, creating it if needed.
