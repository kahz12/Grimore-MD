...
# ``"grimoire.pid"`` (e.g. older scripts). New callers should use
# ``grimoire.utils.paths.daemon_lock_path()`` so the file lives under the
...
def _is_grimoire_process(pid: int) -> bool:
    ...
    daemon dying and a non-Grimoire process inheriting that PID before the
