...
exclude any note already flagged ``grimoire_generated: true`` in
...
``grimoire_generated: true`` so subsequent ``distill`` runs skip it.
...
GENERATED_FLAG_KEY = "grimoire_generated"
...
class Synthesizer:
        ...
        *,
        ...
        if not usable:
            return SynthesisReport(
                ...
                notes_used=0,
                skipped_reason=(
                    "Every matching note is grimoire_generated; "
                    "nothing to distill."
                ),
            )
        ...
        if not sources:
            return SynthesisReport(
                ...
                skipped_reason=(
                    ...
                    "Run `grimoire scan --no-dry-run` first."
                ),
            )
    ...
    def _is_generated_note(path: str) -> bool:
        """Whether the note's frontmatter sets ``grimoire_generated: true``.
